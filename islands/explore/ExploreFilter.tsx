            ...
            <div class="explore-filter">
                <div class="explore-btns sort">
                    <div>
                        ...
                        onClick={() => { openSort.value = !openSort.value }}
                        onFocusOut={() => { !openSort1.value ? openSort.value = true : null}}
                        >
                            <AIcon className="sort-icon" size={16} startPaths={Icons.Sort}/>
                            <p>{props.startSort}</p>
                        </button>

                        <div class="explore-sort dropdown" hidden={openSort} tabindex={0}
                        onMouseEnter={() =>{ openSort.value = false; openSort1.value = true }}
                        onMouseLeave={() =>{ openSort.value = true; openSort1.value = false }}
                        onFocusOut={() => { openSort.value = true }}
                        >
                            <ul>
                                {props.sort.map((option, index) => {
                                    ...
                                    return(
                                        <li key={index}
                                        onClick={() => {
                                            globalThis.location.href = `${url.pathname}?sort=${option.replace(' ', '%20')}`
                                        }}
                                        >
                ...
                />
                <button class="explore-btn filter" tabindex={0} aria-label="Open Sort Menu"
                onClick={() => { openFilter.current!.hidden = !openFilter.current!.hidden }}
                >
        ...
        case "multi-dropdown":
            return(
                <div class={`explore-filters-selection ${props.class}`}>
                    ...
                    <SearchbarDropdown 
                    onChang={props.onChang!} ref={props.ref} class={`explore-filters-selection-item`} 
